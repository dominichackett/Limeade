...
export default function Summary(props) {
 ...
 async function purchasePolicy() {
  try {
    ...
    let transaction = await LimeManagerContract.createPolicy(myPolicy.id,myPolicy.get("name"),
    myPolicy.get("breed"),myPolicy.get("age"),myPolicy.get("coverage"),myPolicy.get("premium")
 ...
 const approveUSDC = async () => {
  ...
  async function getQuote() {
    ...
    pet.save().then(async (p) => {
      ...
      const result = await Moralis.Cloud.run("getQuote",{petData:{petType:p.get("petType"),medicalHistory:props.medicalHistory}})
      ...
      <div className="mt-6 flex  flex-col items-center h-full w-full justify-around py-8 bg-[#CAF46F] bg-opacity-70 rounded-xl sm:w-full">
        ...
        <div className="grid grid-cols-3 gap-4 w-full">
          ...
          <div className="flex flex-col items-center justify-start ">
            ...
            <p className="text-base font-bold">{props.medicalHistory.head?.ear ? "Yes": "No" }</p>
          </div>
         
          
          <div className="flex flex-col items-center justify-start ">
            <p className="text-xl">Kennel Cough</p>
            <p className="text-base font-bold">{props.medicalHistory.head?.cough ? "Yes": "No" }</p>
          </div>

          <div className="flex flex-col items-center justify-start ">
            <p className="text-xl">Eye Issues</p>
            <p className="text-base font-bold">{props.medicalHistory.head?.eye ? "Yes": "No" }</p>
          </div>
          <div className="flex flex-col items-center justify-start ">
            <p className="text-xl">Diarrhea</p>
            <p className="text-base font-bold">{props.medicalHistory.stomach?.diarr ? "Yes": "No" }</p>
          </div>
          <div className="flex flex-col items-center justify-start ">
            <p className="text-xl">Urinary - UTI</p>
            <p className="text-base font-bold">{props.medicalHistory.stomach?.urin ? "Yes": "No" }</p>
          </div>

          <div className="flex flex-col items-center justify-start ">
            <p className="text-xl">Broken Leg</p>
            <p className="text-base font-bold">{props.medicalHistory.leg?.leg ? "Yes": "No" }</p>
          </div>

          <div className="flex flex-col items-center justify-start ">
            <p className="text-xl">Lameness of Limbs</p>
            <p className="text-base font-bold">{props.medicalHistory.leg?.limb ? "Yes": "No" }</p>
          </div>

          <div className="flex flex-col items-center justify-start ">
            <p className="text-xl">Soft Tissue Injury</p>
            <p className="text-base font-bold">{props.medicalHistory.leg?.injury ? "Yes": "No" }</p>
          </div>
          <div className="flex flex-col items-center justify-start ">
            <p className="text-xl">Giardia</p>
            <p className="text-base font-bold">{props.medicalHistory.parasites?.gia ? "Yes": "No" }</p>
          </div>

          <div className="flex flex-col items-center justify-start ">
            <p className="text-xl">Fleas or Ticks</p>
            <p className="text-base font-bold">{props.medicalHistory.parasites?.flea ? "Yes": "No" }</p>
          </div>
          <div className="flex flex-col items-center justify-start ">
            <p className="text-xl">Ear mites</p>
            <p className="text-base font-bold">{props.medicalHistory.parasites?.mites ? "Yes": "No" }</p>
          </div>

          <div className="flex flex-col items-center justify-start ">
            <p className="text-xl">Allergies</p>
            <p className="text-base font-bold">{props.medicalHistory.chronics?.allergies ? "Yes": "No" }</p>
          </div>

          <div className="flex flex-col items-center justify-start ">
            <p className="text-xl">Diabetes</p>
            <p className="text-base font-bold">{props.medicalHistory.chronics?.diabetes ? "Yes": "No" }</p>
          </div>
          <div className="flex flex-col items-center justify-start ">
            <p className="text-xl">Cancer</p>
            <p className="text-base font-bold">{props.medicalHistory.chronics?.cancer ? "Yes": "No" }</p>
          </div>
          <div className="flex flex-col items-center justify-start ">
            <p className="text-xl">Kidney Disease</p>
            <p className="text-base font-bold">{props.medicalHistory.chronics?.kidneydisease ? "Yes": "No" }</p>
          </div>
