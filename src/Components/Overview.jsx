...
import { LimeManagerABI, LimeManagerAddress } from "./Contracts/LimeManagerContract";
import {ethers} from 'ethers'
import Notification from "./Notification/Notification";
import {tokenMetadata} from "../uitls/utils"

export default function Start(props) {
  const { Moralis, user ,isAuthenticated,web3,isWeb3Enabled,enableWeb3} = useMoralis();
 ...
 useEffect(()=>{
  async function getStaked() {
    ...
    {
      const limeManagerContract = new ethers.Contract(LimeManagerAddress,LimeManagerABI,web3.getSigner());
...
useEffect(()=>{
   async function getAgent() {
     ...
     {
       const limeManagerContract = new ethers.Contract(LimeManagerAddress,LimeManagerABI,web3.getSigner());
       let transaction = await limeManagerContract.agent(user.get("ethAddress"));
       setGotAgent(true)
  ...
  useEffect(()=>{
    async function getMember() {
      ...
      {
        const limeManagerContract = new ethers.Contract(LimeManagerAddress,LimeManagerABI,web3.getSigner());
        let transaction = await limeManagerContract.member(user.get("ethAddress"));
        //const value = transaction;
        setGotMember(true)
  ...
  useEffect(() => {
    ...
    query.first().then((result) => {
      setPet({name:result.get("pet").get("name"),petImg:result.get("pet").get("petImg"),gender:result.get("pet").get("gender"),age:result.get("pet").get("age"),breed:result.get("pet").get("breed")});
      console.log(result);
    });
  }, [user]);
    ...
    <main className="flex w-full flex-1 h-full flex-col items-center justify-center px-20 text-center">
      ...
      <div className=" grid grid-cols-3 mt-40 mb-40 gap-4 justify-between">
        ...
        <div className="flex  flex-col h-80 items-center w-full justify-start py-4 bg-[#CAF46F] bg-opacity-70 rounded-xl sm:w-full">
          ...
          <div className="flex flex-col w-6/12 ">
            <div className="flex flex-row items-center justify-around">
              ...
              <p className="text-base font-bold">{pet?.gender?.toUpperCase() }</p>
            </div>
            <div className="flex flex-row items-center justify-around">
        ...
        <div className=" flex  flex-col items-center h-80 w-full justify-start py-6 bg-[#CAF46F] bg-opacity-70 rounded-xl sm:w-full">
          <div className="flex flex-col items-center w-full justify-start">
            ...
            >
             ...
             {claim.get("state") == 0 &&  <ExclamationCircleIcon className="h-5 text-orange-500" />}

             {claim.get("state") == 1 && <CheckCircleIcon className="h-5 text-green-500" />}
             {claim.get("state") == 2 &&  <XCircleIcon className="h-5 text-red-500" />
}
            </div>
        ...
        <div className="flex  flex-col items-center h-80 w-full justify-start py-6 bg-[#CAF46F] bg-opacity-70 rounded-xl sm:w-full">
          ...
          {/* Verify Member */}
          <div hidden={member==true}   className="flex  flex-col h-80 items-center w-full justify-start py-6 bg-[#CAF46F] bg-opacity-70 rounded-xl sm:w-full">
          <div    className="flex flex-col items-center justify-start">
            <div
          ...
          <div className="py-2"  hidden={ gotMember==false || (gotMember==true && member==true) || (gotPolygonIdEnabled==true && polygonIdEnabled == true)}>
          <button
          onClick={joinDOA}
...
Become a Member </button>
          ...
          <div hidden={member==false} className="py-2" >
           <img height={200} width={200} alt="Active Member" src={memberMetadata?.metadata.image}/>
           
          </div>

          <div className="py-2" hidden={ gotMember==false || (gotMember==true && member==true) || (gotPolygonIdEnabled==true && polygonIdEnabled == false)}
             >
            <Image 
          ...
          <div hidden={agent==true}  className="flex flex-col items-center justify-start">
            <div
          ...
          <div hidden={ gotAgent==false || (gotAgent==true && agent==true) || (gotPolygonIdEnabled==true && polygonIdEnabled == false)}
          ...
          <div hidden={agent==false} className="py-2" >
           <img height={200} width={200} alt="Active Agent" src={agentMetadata?.metadata.image}/>
           
          </div>
          <div className="py-2"  hidden={ gotAgent==false || (gotAgent==true && agent==true) || (gotPolygonIdEnabled==true && polygonIdEnabled == true)}>
          <button
               onClick={becomeAgent}
               disabled={agent==true}
  className="flex flex-col items-center justify-center w-40 h-12 bg-black text-white rounded-full"
